...
/**
 * Owns one JDBC connection and restores its original catalog and schema before the connection can
...
public final class JdbcTargetScope implements AutoCloseable {
  ...
  private JdbcTargetScope(Connection connection, String originalCatalog, String originalSchema) {
    this.connection = connection;
    this.originalCatalog = originalCatalog;
    this.originalSchema = originalSchema;
  }

  public static JdbcTargetScope open(DataSource dataSource, DatabaseTarget target)
      throws SQLException {
    ...
    try {
      scope = new JdbcTargetScope(connection, connection.getCatalog(), connection.getSchema());
  ...
  private static void cleanupFailedOpen(
      JdbcTargetScope scope, Connection connection, Throwable failure) {
    try {
      if (scope != null) {
        scope.close();
      } else {
