...
public final class LiquibaseMigrationEngine implements DatabaseMigrationEngine {
  ...
  public MigrationValidation validate(MigrationRequest request) {
    return execute(
        ...
        liquibase -> {
          ...
          try {
            liquibase.validate();
            return MigrationValidation.valid(request.plan().id(), request.target(), validatedAt);
          } catch (LiquibaseException exception) {
            ValidationFailedException validationFailure = findValidationFailure(exception);
            ...
            return MigrationValidation.invalid(
                request.plan().id(), request.target(), validatedAt, List.copyOf(errors));
          }
        });
  }
  ...
  public MigrationStatus status(MigrationRequest request) {
    return execute(request, MigrationOperation.STATUS, liquibase -> inspect(request, liquibase));
  }
  ...
  private MigrationStatus inspect(MigrationRequest request, Liquibase liquibase)
      throws LiquibaseException {
    List<MigrationChange> changes = new ArrayList<>();
    for (ChangeSetStatus status :
        liquibase.getChangeSetStatuses(contexts(request), labels(request))) {
      ChangeSet changeSet = status.getChangeSet();
      CheckSum checksum =
          status.getStoredCheckSum() != null
      ...
      changes.add(
          new MigrationChange(
              ...
              Optional.ofNullable(status.getDateLastExecuted()).map(date -> date.toInstant()),
              state));
    }
    ...
    try {
      ...
      Database database =
          DatabaseFactory.getInstance()
              .findCorrectDatabaseImplementation(new JdbcConnection(scope.connection()));
      request
          .target()
          .catalog()
          .ifPresent(
  ...
  private MigrationException migrationException(
      MigrationOperation operation, MigrationRequest request, Throwable cause) {
    return new MigrationException(operation, request.plan().id(), request.target(), cause);
  }
  ...
  private static final class DatabaseConfigurationException extends RuntimeException {
